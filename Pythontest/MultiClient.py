...
cf = ConfigParser()
f = open("ClientData.ini", encoding="utf-8")
cf.read_file(f)
        ...
        if hello_str[0] == "PlayAlert":
            ...
            if hello_str[1] == "LoginType":
                if hello_str[2] == "True":
                    ...
                    gui.listBox.insert(END, gui.entryusername.get()+"成功登录"+hello_str[3])
                    ...
                    gui.listBox.insert(END, gui.entryusername.get()+"登录不成功"+hello_str[3])
                    #messagebox.showinfo("提示", "登录不成功")
        elif hello_str[0] == "UserLogout":
            if hello_str[1] == "LogoutType":
                if hello_str[2] == "True":
                    #Labelusernamestatus.config(text="注销成功")
                    gui.listBox.insert(END, gui.entryusername.get() + "注销成功"+hello_str[3])
                elif hello_str[2] == "False":
                    #Labelusernamestatus.config(text="注销失败")
                    gui.listBox.insert(END, gui.entryusername.get() + "注销失败"+hello_str[3])
...
class GUI:
    ...
    def __init__(self, argroot):
        ...
        self.image_file = PhotoImage(file="Img/60x60.png")
    ...
    def logout(self):
        ...
        strwillsend="UserLogout###111###222"
        print(strwillsend)
        globalsocket.send(strwillsend.encode())
...
def CallbackClose():
    if tkinter.messagebox.askyesno(title='警告',message='确认关闭吗？'):
        if tkinter.messagebox.askyesno(title='警告', message='永久保存账号信息吗？'):
            mycf = ConfigParser()
            # 先读出来
            mycf.read("ClientData.ini", encoding="utf-8")
            # fileOpenmycf = open("ClientData.ini")
            strentryusername=gui.entryusername.get()
            strentryuserPassword = gui.entryuserPassword.get()
            mycf.set("UserAccount", "mobile", str(strentryusername))
            mycf.set("UserAccount", "passwordencode", str(strentryuserPassword))
